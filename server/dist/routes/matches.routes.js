...
router.get('/', auth_1.requireAuth, (0, validate_1.validate)(schemas_1.matchQuerySchema, 'query'), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const result = await matchService.getMatchesForUser(req.user.userId, req.query);
    (0, responses_1.ok)(res, result);
}));
router.get('/:userId', auth_1.requireAuth, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const detail = await matchService.getMatchDetail(req.user.userId, req.params.userId);
    (0, responses_1.ok)(res, detail);
}));
